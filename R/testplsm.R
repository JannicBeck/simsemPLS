test_plsm <- function(object, scoeffs, mcoeffs, sresid, mresid, FUN){
    ...
    assign(paste("test.",substitute(object),".nobs", sep = ""),
           simplsm(object, 100, nobs = 250, scoeffs = scoeffs, mcoeffs = mcoeffs),
           envir = .GlobalEnv)
    
    # test with high observations
    assign(paste("test.",substitute(object),".nobs.high", sep = ""), 
           simplsm(object, 100, nobs = 1000, scoeffs = scoeffs, mcoeffs = mcoeffs),
           envir = .GlobalEnv)
    ...
    assign(paste("test.",substitute(object),".sresid", sep = ""), 
           simplsm(object, 100, scoeffs = scoeffs, mcoeffs = mcoeffs, sresid = sresid),
           envir = .GlobalEnv)
    
    # test with measuremod residuals
    assign(paste("test.",substitute(object),".mresid", sep = ""), 
           simplsm(object, 100, scoeffs = scoeffs, mcoeffs = mcoeffs, mresid = mresid),
           envir = .GlobalEnv)
    
    # test with residuals
    assign(paste("test.",substitute(object),".resid", sep = ""), 
           simplsm(object, 100, scoeffs = scoeffs, mcoeffs = mcoeffs, sresid = sresid, mresid = mresid),
           envir = .GlobalEnv)
